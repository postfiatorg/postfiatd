...
OrchardBundleWrapper::parse(Slice const& data)
{
    try
    {
        ...
        auto bundle = orchard_postfiat::ffi::orchard_bundle_parse(rust_data);

        // Wrap in our C++ wrapper
        auto wrapper = std::unique_ptr<rust::Box<orchard_postfiat::ffi::OrchardBundle>>(
            new rust::Box<orchard_postfiat::ffi::OrchardBundle>(std::move(bundle))
        );
...
{
    auto rust_vec = orchard_postfiat::ffi::orchard_bundle_serialize(**inner_);
...
{
    return orchard_postfiat::ffi::orchard_bundle_is_present(**inner_);
}

bool
OrchardBundleWrapper::isValid() const
{
    return orchard_postfiat::ffi::orchard_bundle_is_valid(**inner_);
}

std::int64_t
OrchardBundleWrapper::getValueBalance() const
{
    return orchard_postfiat::ffi::orchard_bundle_get_value_balance(**inner_);
}

uint256
OrchardBundleWrapper::getAnchor() const
{
    auto anchor_bytes = orchard_postfiat::ffi::orchard_bundle_get_anchor(**inner_);
...
OrchardBundleWrapper::getNullifiers() const
{
    ...
    auto rust_nullifiers_flat = orchard_postfiat::ffi::orchard_bundle_get_nullifiers(**inner_);
...
{
    return orchard_postfiat::ffi::orchard_bundle_num_actions(**inner_);
}
...
OrchardBundleWrapper::verifyProof(uint256 const& sighash) const
{
    ...
    return orchard_postfiat::ffi::orchard_verify_bundle_proof(
        **inner_,
        sighash_bytes
    );
}
...
OrchardBatchVerifier::OrchardBatchVerifier()
    : inner_(new rust::Box<orchard_postfiat::ffi::OrchardBatchVerifier>(
        orchard_postfiat::ffi::orchard_batch_verify_init()))
{
}
    ...
    uint256 const& sighash)
{
    ...
    auto bundle_clone = orchard_postfiat::ffi::orchard_bundle_box_clone(**bundle.getRustBundle());

    orchard_postfiat::ffi::orchard_batch_verify_add(
        ***inner_,
...
{
    return orchard_postfiat::ffi::orchard_batch_verify_finalize(
        std::move(**inner_)
    );
}
