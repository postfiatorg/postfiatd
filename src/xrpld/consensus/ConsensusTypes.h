...
/** Offset of the network's close time relative to ours, using a weighted
    median.
    ...
    earlier bin. A median resists a minority of peers reporting far-off
    close times, which a mean does not.

    @param times Our own close time and the weighted close times of peers.
    @return Weighted median of all close times minus our own, in whole
            seconds.
*/
...
medianCloseOffset(ConsensusCloseTimes const& times)
{
    ...
    auto step = [&](time_point t, std::int64_t w) {
        XRPL_ASSERT(
            tally < halfWeight,
            "ripple::medianCloseOffset::step : median not yet found");
        tally += w;
        if (tally >= halfWeight)
        {
            median = t;
    ...
    {
        // LCOV_EXCL_START
        UNREACHABLE("ripple::medianCloseOffset : median not found");
        median = times.self;
        // LCOV_EXCL_STOP
    }
