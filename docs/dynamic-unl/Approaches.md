...
**Data collection:** The foundation collects validator performance data from VHS, MaxMind GeoIP2, and SumSub. The resulting data snapshot is published to IPFS. Validators can cross-check telemetry data against their own observations, and third-party data (MaxMind, SumSub) can be independently verified.
...
**IPFS audit trail:** Every scoring round publishes the full audit trail to IPFS — data snapshot, scoring outputs, and final UNL. Content-addressed and independently verifiable.

**Identity gate:** Validators must pass KYC/KYB verification (via SumSub) before they can be scored. Institutional domain verification is optional but positively influences scores.
...
**Cost to foundation:** LLM API fees per round (2 calls × model pricing), IPFS pinning, infrastructure hosting.
...
**Community impact:** Easiest to understand. Community members can verify every scoring round by reading the LLM reasoning on IPFS. However, participation is passive — community members cannot independently score, only audit. Trust depends on the foundation's reputation and transparency of the published audit trail.
...
Multiple independent Oracle Nodes each call a cloud LLM API with the same data and prompts. Oracle Nodes submit scores via commit-reveal on-chain. Final scores are computed by taking the median across all valid submissions.

**Trust model:** Distributed across Oracle Nodes. No single entity controls the scoring outcome. The foundation still collects data, but it cannot control individual Oracle Node scores. Median aggregation resists outlier manipulation.

**Complexity:** High. Requires Oracle Node registration with PFT bond, commit-reveal protocol, quorum rules, aggregation service, liveness tracking, and dispute handling. Significantly more infrastructure than Approach 1.

**Time to ship:** Medium. Builds on Approach 1 infrastructure but adds substantial distributed coordination.

**Centralization:** Moderately decentralized. Multiple independent scorers eliminate single-entity control over scores. However, all nodes depend on cloud LLM API providers (if the provider goes down, no one can score).

**Cost to foundation:** Data collection, aggregation service, IPFS pinning. No LLM API fees (Oracle Nodes pay their own).
...
**Key risk:** Requires enough Oracle Nodes to participate for quorum. All nodes depend on cloud API availability. If the LLM provider has an outage during a scoring round, the round fails.
...
**Centralization:** Most decentralized. Multiple independent scorers, no cloud API dependency, computation is verifiable. The foundation's role is reduced to data collection, aggregation (deterministic, anyone can recompute), and infrastructure operation. The closest to a fully trustless scoring system.

**Cost to foundation:** Data collection, aggregation service, IPFS pinning. Minimal compared to other approaches — no API fees, no GPU infrastructure for scoring.
...
| **How proven** | Published audit trail | Commit-reveal + median | Proof-of-logits | Proof-of-logits |
...
| **Cost to foundation** | API fees + data collection | Data collection + aggregation | GPU hardware + data collection | Data collection only |
...
| Dimension | Approach 1 | Approach 2 | Approach 3 | Approach 4 |
|-----------|-----------|-----------|-----------|-----------|
| **Can verify scores** | Yes — read published reasoning | Yes — read published reasoning | Yes — spot-check logits (needs GPU) | Yes — spot-check logits (needs GPU) |
...
| **Proof maturity** | N/A (trust-based) | N/A (trust-based) | Unproven (proof-of-logits) | Unproven (proof-of-logits) |
...
| **Institutions (conservative)** | Approach 2 | Distributed trust with proven coordination mechanisms | Approach 1 for simplicity |
